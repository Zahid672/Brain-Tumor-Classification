...
if __name__ == "__main__":
    ...
    for key, value in   tqdm(model_dict.items()):           #model_dict.items(): 
        ...
        if key == inception_v3:
            ...
            train_dataset = CustomDataset(data_dir, 'train', transform=transforms, size=(299, 299))
            test_dataset = CustomDataset(data_dir, 'test', transform=transforms, size=(299, 299))
        
        
        # elif key.split('_')[-1] == '384':
        #     print('Using vit_base_patch16_384')
       
        #     train_dataset = CustomDataset(data_dir, 'train', transform=transforms, size=(384, 384))
        #     test_dataset = CustomDataset(data_dir, 'test', transform=transforms, size=(384, 384))
        else:
            train_dataset = CustomDataset(data_dir, 'train', transform=transforms, size=(224, 224))
            test_dataset = CustomDataset(data_dir, 'test', transform=transforms, size=(224, 224))
        ...
        sub_dir = os.path.join(main_dir, key.__name__) # for cnn models, use just key for vit models
